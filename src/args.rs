...
pub struct Args {
    ...
    ///   async=[true|false]
    ///
    ///   Asynchronously upload backend responses without waiting for other backends.
    ///   If an asynchronous upload fails, there will be no indiciation sent to the
    ///   bes client. By default, client responses are blocked on receiving responses
    ///   from all backends (async = false).
    ///
    ///
